...
/* gives the factorial number of the positif int n */
int factorial_bis(int n) {
    if (n < 0) return 0;

    if (n == 1)
        return n;

    return n * factorial_bis(n - 1);
}

/* deal with the factorial */
void factorial(void){
    int a;
...
void mod(void){
    ...
    display_operation('$');
    display_stack();
    display_action_msg("Get the modulo");
    actualise_window();
    pause_keyboard();
